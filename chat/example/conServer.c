...
int main(int argc, char **argv[]) {
	...
	init.sin_addr.s_addr = htonl(INADDR_ANY);
	init.sin_port = htons(0);
	...
	Conn *connList[CONNECTIONS]; // concurrent connections
	...
	for(i = 0; i < CONNECTIONS; i++) {
		if(i == 0) { // Special case for initial fd
			...
			bind(temp->clifd,(struct sockaddr *)&init,sizeof(init));
